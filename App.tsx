...
export default function App() {
  ...
  const [notice, setNotice] = useState('Tap any button to see what it does.');

  const missingCount = useMemo(() => documents.filter((item) => item.status === 'Missing').length, [documents]);

  function uploadMissingDocument() {
    const nextDocuments = documents.map((item) =>
      item.status === 'Missing' ? { ...item, status: 'Uploaded' as DocStatus } : item
    );
    setDocuments(nextDocuments);
    setProgress(58);
    setStage('Documents uploaded');
    setNotice('Document upload simulated. Missing documents were marked as uploaded.');
    setTab('Documents');
  }

  function requestUpdate() {
    setNotice('Status request sent. In production this would create a message to the claims office.');
    setTab('Messages');
  }

  function sendMessage() {
    setMessageSent(true);
    setNotice('Message sent. The demo added your message to the thread.');
  }
  ...
  function resetDemo() {
    ...
    setNotice('Demo reset to the original claim state.');
    setTab('Home');
  }
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.page}>
        <View style={styles.hero}>
          ...
          <Text style={styles.copy}>Interactive demo for claim tracking, documents, messages, and profile actions.</Text>
        </View>

        <View style={styles.notice}><Text style={styles.noticeText}>{notice}</Text></View>

        <View style={styles.tabs}>
          {(['Home', 'Documents', 'Messages', 'Profile'] as Tab[]).map((item) => (
            <TouchableOpacity key={item} onPress={() => setTab(item)} style={[styles.tab, tab === item && styles.tabActive]}>
        ...
        {tab === 'Home' && (
          <View style={styles.stack}>
            <View style={styles.grid}>
              ...
              <Card label="Missing docs" value={missingCount.toString()} />
              <Card label="Messages" value={messageSent ? '2' : '1'} />
            </View>
            <Panel title="Current status">
              <Row label="Stage" value={stage} />
              <Row label="Office" value="Johannesburg Regional Office" />
              <Row label="Next action" value={missingCount > 0 ? 'Upload missing documents' : 'Wait for review'} />
            </Panel>
            <Button label="Upload missing documents" onPress={uploadMissingDocument} />
            <Button label="Request status update" onPress={requestUpdate} secondary />
          </View>
        )}

        {tab === 'Documents' && (
          <Panel title="Documents">
            {documents.map((item) => <Row key={item.name} label={item.name} value={item.status} />)}
            <Button label="Simulate upload" onPress={uploadMissingDocument} />
          </Panel>
        )}

        {tab === 'Messages' && (
          <Panel title="Messages">
            <Row label="RAF Claims Office" value="Please upload the accident report." />
            {messageSent && <Row label="You" value="Please confirm my latest claim status." />}
            <Button label="Send status message" onPress={sendMessage} />
          </Panel>
        )}

        {tab === 'Profile' && (
          <Panel title="Profile and consent">
            <Row label="Claimant" value="Demo Claimant" />
            <Row label="POPIA consent" value="Accepted" />
            <Row label="Identity" value="Verified placeholder" />
            <Button label="Reset demo" onPress={resetDemo} secondary />
          </Panel>
        )}
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  noticeText: { color: '#0F2D3A', fontWeight: '800', lineHeight: 21 },
  tabs: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  tab: { paddingHorizontal: 14, paddingVertical: 10, borderRadius: 999, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#DDE6EB' },
  tabActive: { backgroundColor: '#0F6B8D', borderColor: '#0F6B8D' },
  tabText: { color: '#61717C', fontWeight: '900' },
